...
class User(Avatar):
    ...
    def __init__(
        self, playername: str, server: Server.Server, client: RemoteReference
    ):
    ...
    def perspective_get_playername(self, arg: Any) -> str:
    ...
    def perspective_send_chat_message(
        ...
        """
        self.server.send_chat_message(self.playername, dest, text)
    ...
    def perspective_join_game(
        ...
        return self.server.join_game(
            self.playername, game_name, player_class, player_info
        )

    def perspective_start_game(self, game_name: str) -> None:
        self.server.start_game(self.playername, game_name)

    def perspective_pick_color(self, game_name: str, color: str) -> None:
        self.server.pick_color(self.playername, game_name, color)

    def perspective_pick_first_marker(
        self, game_name: str, markerid: str
    ) -> None:
        self.server.pick_first_marker(self.playername, game_name, markerid)
    ...
    def perspective_done_with_splits(self, game_name: str) -> None:
        self.server.done_with_splits(self.playername, game_name)

    def perspective_take_mulligan(self, game_name: str) -> None:
        self.server.take_mulligan(self.playername, game_name)
    ...
    def perspective_done_with_moves(self, game_name: str) -> None:
        self.server.done_with_moves(self.playername, game_name)

    def perspective_resolve_engagement(
        self, game_name: str, hexlabel: int
    ) -> None:
        self.server.resolve_engagement(self.playername, game_name, hexlabel)

    def perspective_flee(self, game_name: str, markerid: str) -> None:
        self.server.flee(self.playername, game_name, markerid)

    def perspective_do_not_flee(self, game_name: str, markerid: str) -> None:
        self.server.do_not_flee(self.playername, game_name, markerid)
    ...
    def perspective_done_with_reinforcements(self, game_name: str) -> None:
        self.server.done_with_reinforcements(self.playername, game_name)

    def perspective_done_with_maneuvers(self, game_name: str) -> None:
        self.server.done_with_maneuvers(self.playername, game_name)

    def perspective_done_with_strikes(self, game_name: str) -> None:
        self.server.done_with_strikes(self.playername, game_name)

    def perspective_done_with_counterstrikes(self, game_name: str) -> None:
        self.server.done_with_counterstrikes(self.playername, game_name)
    ...
    def perspective_acquire_angels(
        ...
        self.server.acquire_angels(
            self.playername, game_name, markerid, angel_names
        )
    ...
    def perspective_do_not_acquire_angels(
        ...
        self.server.do_not_acquire_angels(self.playername, game_name, markerid)

    def perspective_done_with_engagements(self, game_name: str) -> None:
        self.server.done_with_engagements(self.playername, game_name)
    ...
    def perspective_done_with_recruits(self, game_name: str) -> None:
        self.server.done_with_recruits(self.playername, game_name)
    ...
    def perspective_do_not_summon_angel(
        ...
        self.server.do_not_summon_angel(self.playername, game_name, markerid)

    def perspective_do_not_reinforce(
        self, game_name: str, markerid: str
    ) -> None:
        self.server.do_not_reinforce(self.playername, game_name, markerid)
    ...
    def perspective_apply_action(self, action: Action.Action) -> None:
        ...
        if isinstance(action, Action.UndoSplit):
            ...
            self.server.undo_move_legion(
                self.playername, action.game_name, action.markerid
            )
    ...
    def perspective_save(self, game_name: str) -> None:
        self.server.save(self.playername, game_name)

    def perspective_withdraw(self, game_name: str) -> None:
        self.server.withdraw(self.playername, game_name)

    def perspective_pause_ai(self, game_name: str) -> None:
        self.server.pause_ai(self.playername, game_name)

    def perspective_resume_ai(self, game_name: str) -> None:
        self.server.resume_ai(self.playername, game_name)
    ...
    def __repr__(self) -> str:
        return "User " + self.playername

    def add_observer(self, mind: Any) -> None:
        logging.info(f"User.add_observer {self} {mind}")
        def1 = self.client.callRemote("set_playername", self.playername)  # type: ignore
    ...
    def trap_connection_lost(self, failure: Failure) -> None:
        ...
        if not self.logging_out:
            ...
            logging.info(f"Connection lost for {self.playername}; logging out")
            self.logout()
    ...
    def logout(self) -> None:
        logging.info(f"logout {self.playername}")
